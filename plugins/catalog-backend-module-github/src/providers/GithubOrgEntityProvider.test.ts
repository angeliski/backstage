...
describe('GithubOrgEntityProvider', () => {
  ...
  describe('receiving events from github', () => {
    ...
    it('should apply refresh on receive a edited team', async () => {
      const entityProviderConnection: EntityProviderConnection = {
        applyMutation: jest.fn(),
        refresh: jest.fn(),
      };
      ...
      expect(entityProviderConnection.refresh).toHaveBeenCalledWith({
        keys: ['url:https://github.com/orgs/test-org/teams/new-team'],
      });
    });

    it('should apply refresh on receive a membership added', async () => {
      const entityProviderConnection: EntityProviderConnection = {
        applyMutation: jest.fn(),
        refresh: jest.fn(),
      };
    ...
    it('should apply refresh on receive a membership removed', async () => {
      const entityProviderConnection: EntityProviderConnection = {
        applyMutation: jest.fn(),
        refresh: jest.fn(),
      };
