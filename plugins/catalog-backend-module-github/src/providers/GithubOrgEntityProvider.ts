  ...
  implements EntityProvider, EventSubscriber
{
  ...
  async onEvent(params: EventParams): Promise<void> {
    ...
    if (params.topic.includes('team')) {
      ...
      if (teamEvent.action === 'created') {
        ...
        await this.onTeamEditedInOrganization(teamEvent);
      }
    }
    ...
    if (params.topic.includes('membership')) {
      ...
      await this.onMemberChangeToTeam(membershipEvent);
    }
  ...
  private async onTeamEditedInOrganization(event: TeamEvent) {
    if (!this.connection) {
      throw new Error('Not initialized');
    }

    const { slug } = event.team;
    const org = event.organization.login;

    await this.connection.refresh({
      keys: [
        `url:https://${this.options.gitHubConfig.host}/orgs/${org}/teams/${slug}`,
      ],
    });
  }
  ...
  private async onMemberChangeToTeam(event: MembershipEvent) {
    if (!this.connection) {
      throw new Error('Not initialized');
    }
    ...
    const { slug: teamSlug } = event.team;
    const { login } = event.member;
    const org = event.organization.login;

    await this.connection.refresh({
      keys: [
        `url:https://${this.options.gitHubConfig.host}/orgs/${org}/teams/${teamSlug}`,
        `url:https://${this.options.gitHubConfig.host}/${login}`,
      ],
    });
  }
